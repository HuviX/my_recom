...
def get_prob_features(
    ...
    def prob_features(row):
        ...
        return pd.Series([ovr_cart, ovr_view, prob_cart[0], prob_view[0], max_cart, max_view])
    
    df[
        [
            'pred_cart_sim', 'pred_view_sim',
            'prob_cart', 'prob_view',
            'max_cart_sim', 'max_view_sim'
        ]
    ] = df[
        ['view', 'to_cart', 'prediction']
    ].apply(prob_features, axis=1)

    df[['prob_cart', 'prob_view']] = df[['prob_cart', 'prob_view']].fillna(0.)
    return df
...
def category_mean_encoding(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cumsum = train_df.groupby('category_1')['target'].cumsum() - train_df['target']
    ...
    test_df['cat1_encoded_feature'] = test_df['category_1'].map(cat1_target_mean)
    test_df['cat1_encoded_feature'].fillna(global_mean, inplace = True)

    cat2_target_mean = train_df.groupby('category_2')['target'].mean()
    cumsum = train_df.groupby('category_2')['target'].cumsum() - train_df['target']
    ...
    test_df['cat2_encoded_feature'] = test_df['category_2'].map(cat2_target_mean)
...
def main(
    ...
    **kwargs,
    ...
    candidates_df['len_of_w2vec'] = candidates_df['w2vec_pred'].apply(lambda x: len(x))
    candidates_df['len_of_cos'] = candidates_df['cos_pred'].apply(lambda x: len(x))
    candidates_df['len_of_pred'] = candidates_df['len_of_cos'] + candidates_df['len_of_w2vec']

    candidates_df['ovr_pred'] = candidates_df['w2vec_pred'] + candidates_df['cos_pred']
