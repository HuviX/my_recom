...
def get_mapping(
    ...
    item_mapping = pd.DataFrame(
        {"old": uniq_items, "new": np.arange(uniq_items.shape[0])}
    )
...
def main(
    ...
    **kwargs,
    ...
    for action_type in ["view", "order", "to_cart"]:
        ...
        cntr = Counter(un)
        with open(
            f"{data_dir}/num_of_" + action_type + ".pkl", "wb"
        ) as handle:
    ...
    for action_type in ["view", "order", "to_cart"]:
        ...
        if action_type in ["view", "order", "to_cart"]:
            (
                action_column.map(lambda x: [str(i) for i in x]).to_pickle(
                    f"{w2vec_data_dir}/{action_type}_df.pkl", protocol=3
                )
            )
